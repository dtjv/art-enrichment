var groupBy        = require('js-utils').groupBy;
var Course         = require('./course');
var Browser        = require('./globals/browser');
var SpreadsheetApp = require('./globals/spreadsheet');
...
/**
 * Accesses the target spreadsheet, adds additional calculations and then loads the data.
 *
 * @return {ConflictApp} This application for chaining.
 */
ConflictApp.prototype.setup = function () {
  // Get a handle to the data sheet, activate it and clear the background.
  var dSheet = SpreadsheetApp
    .getActive()
  ...
  dSheet
    ...
    .setFormulas(ConflictApp._setRangeValues(dRows, 1, this.FORMULA_END_TIME, this.DATA_ROW, 0));
  dSheet
    .getRange(this.DATA_ROW, this.TOOLS, dRows)
    .setFormulas(ConflictApp._setRangeValues(dRows, 1, this.FORMULA_TOOLS, this.DATA_ROW, 0));
 ...
 * @return {string}     The hex color with hash sign between `min` (included) and `max` (excluded).
 */
ConflictApp._getRandomColor = function (min, max) {
  return '#' + ConflictApp._getRandomInt(min, max).toString(16);
};
 ...
 * @return {boolean}            Return true if `course` is in conflict with any other course in `collection`.
 */
ConflictApp._filterConflicts = function (course, idx, collection) {
  var courses = collection.slice(idx + 1);

  for (var i = 0; i < courses.length; i += 1) {
    if (course.end >= courses[i].start &&
...
/**
 * Finds courses in conflicts. Then sets this application's `conflicts` property the array of conflicts.
 *
 * @return {ConflictApp} This application for chaining.
 */
ConflictApp.prototype.getConflicts = function () {
  ...
  for (var key in coursesByDate) {
    if (coursesByDate.hasOwnProperty(key)) {
      ...
      conflicts = conflicts.concat(coursesByDate[key]
 ...
 * @param {number} rowOffSet If `value` is being interpolated for `{{row}}`, this sets the start value of row.
 * @param {number} colOffSet If `value` is being interpolated for `{{col}}`, this sets the start value of col.
 * @return {Array}           A new, 2d array of values with corresponding row/col numbers if interpolation was requested.
 */
ConflictApp._setRangeValues = function (rows, cols, value, rowOffSet, colOffSet) {
  var row;
  var result = [];
 ...
 */
ConflictApp._getRandomInt = function (min, max) {
  return Math.floor(Math.random() * (max - min)) + min;
};

module.exports = ConflictApp;
